 ...
 */
public class ProposalGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all proposals").get("/api/proposals").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new proposal")
                        .post("/api/proposals")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"title\": \"SAMPLE_TEXT\"" +
                                ", \"proposalAbstract\": \"SAMPLE_TEXT\"" +
                                ", \"submissionDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"methodology\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"SUBMITTED\"" +
                                ", \"plagiarismScore\": 0" +
                                "}"
                            )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created proposal").get("${new_proposal_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created proposal").delete("${new_proposal_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Proposal entity").exec(scn);
